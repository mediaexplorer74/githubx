            ...
            _ => blobCache.GetAllKeys()
                .SelectMany(x => x
                    .Where(y => y.StartsWith(
                        GetTypePrefixedKey(string.Empty, typeof(T)), StringComparison.CurrentCultureIgnoreCase))
                    .ToObservable())
                .SelectMany(x =>
...
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    public static IObservable<Unit> InvalidateAllObjects<T>(this IBlobCache blobCache)
    {
        ...
        blobCache.GetAllKeys()
            .SelectMany(x =>
                x.Where(y => y.StartsWith(GetTypePrefixedKey(string.Empty, typeof(T)), StringComparison.CurrentCultureIgnoreCase))
                    .ToObservable())
