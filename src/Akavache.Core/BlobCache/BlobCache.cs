...
public static class BlobCache
{
    ...
    static BlobCache()
    {
        Locator.RegisterResolverCallbackChanged(() =>
        {
            ...
            Locator.CurrentMutable.InitializeAkavache((IReadonlyDependencyResolver)Locator.Current);
        });
    ...
    public static IBlobCache? UserAccount
    {
        ...
        {
            /*
            return _unitTestUserAccount 
                ?? _userAccount 
                ?? (_shutdownRequested ? new ShutdownBlobCache() : null) 
                ?? Locator.Current.GetService<IBlobCache>("UserAccount") 
                ?? throw new InvalidOperationException(
                    "Unable to resolve UserAccount cache. Make sure Akavache is initialized properly.");
            */
            IBlobCache? l = default;//Locator.Current.GetService<IBlobCache>("UserAccount");
            return l;
        }
