...
namespace GithubX.Shared.Services
{
	public class UserService
	{
		...
		{
			return userName != null ? client?.User?.Get(userName) : client?.User?.Current();
		}
		...
		public async Task<IReadOnlyList<User>> GetAllFollowing(ApiOptions options, 
			string? user = null)
			=> user == null
			? await client.User.Followers.GetAllFollowingForCurrent(options)
			: await client.User.Followers.GetAllFollowing(user, options);

		public async Task<IReadOnlyList<User>> GetAllFollowers(ApiOptions options, 
			string? user = null)
			=> user == null
		...
		#region Notification
		public async Task<IReadOnlyList<Octokit.Notification>> GetAllNotifications(ApiOptions options)
		{
			return await client.Activity.Notifications.GetAllForCurrent(new NotificationsRequest
			{ All = true }, options);
		}
